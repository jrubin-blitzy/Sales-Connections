...
export const router = createBrowserRouter([
  // =========================================================================
  // Public authentication surface (NOT wrapped in <ProtectedRoute>)
  // =========================================================================
  ...
  // =========================================================================
  // Authenticated surface (each route requires a session via ProtectedRoute)
  // =========================================================================

  // Index route: the bare "/" redirects to /feed. Wrapped in <ProtectedRoute>
  // so unauthenticated visitors are bounced to /login first; the Navigate
  // only fires for authenticated users, who then land on /feed (the AAP-
  // specified default landing surface for Viewer and Contributor roles per
  // AAP Sec 0.5.4).
  {
    path: "/",
    element: (
  ...
  // F-004 Connection Feed / Dashboard: the primary surface for Sales Reps
  // (Viewer role) and Contributors. Filter and sort state is encoded in URL
  // search params (handled inside the ConnectionFeed component via
  // useSearchParams), NOT as separate routes - keeping the route table
  // minimal and the URL shareable.
  {
    path: "/feed",
    element: (
      <ProtectedRoute>
  ...
  // F-001 Add Connection: mounts AddEditConnectionForm in create mode. The
  // component reads its `mode` prop to decide whether to load an existing
  // record (mode="edit") or start with empty state (mode="create").
  {
  ...
  // F-011 Connection Detail: shows all nine record fields plus the edit-
  // history feed (sourced from audit_events via GET /api/connections/:id/history).
  // The :id parameter is read inside the component via useParams.
  {
    path: "/connections/:id",
    element: (
      <ProtectedRoute>
  ...
  // mode. The component reads :id via useParams and hydrates the form by
  // fetching GET /api/connections/:id. Edit authorization is enforced by
  // the backend (own record for Contributor, any record for Admin); the
  // UI does not gate this route because Contributors should be able to
  // navigate to their own records' edit form.
  {
  ...
  // F-014 Admin Panel: nested route tree. The /admin root mounts AdminPanel
  // (header banner + tab navigation + <Outlet />), and the children render
  // inside the outlet. Index route redirects /admin -> /admin/users so the
  // user lands on a populated tab immediately rather than seeing an empty
  // outlet, matching the AAP Sec 0.5.4 description.
  //
  // RoleGate's `fallback` prop renders <Navigate to="/feed" replace /> for
  // non-Admin sessions, surfacing the "you cannot view this surface" outcome
  // as a route redirect rather than a blank page. The backend's
  // @requires_role(Admin) decorator on every /api/admin/* endpoint is the
  // authoritative gate (per AAP Sec 0.7.1 invariant 7); this RoleGate is
  // a UI courtesy that prevents the broken admin UI from rendering for
  // users who would only see 403 errors anyway.
  {
    path: "/admin",
    element: (
      <ProtectedRoute>
    ...
    children: [
      ...
      // F-007 / F-014 Record moderation: includes soft-deleted records and
      // the Admin-only hard-delete control.
