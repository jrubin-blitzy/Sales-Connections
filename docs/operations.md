...
5. In production, `RUN_MIGRATIONS=false` on the long-running backend tasks. The Terraform `infra/terraform/modules/ecs/migration.tf` resource provisions a dedicated `<name_prefix>-migration` ECS task definition (e.g., `sales-connections-prod-migration`); the `.github/workflows/cd.yml` CD pipeline invokes `aws ecs run-task` against that task definition with `RUN_MIGRATIONS=true` and the new image SHA BEFORE the rolling deployment of the long-running tasks.
6. Migration failures abort the deployment. The one-shot task exits non-zero, the CD workflow's exit-code check fires `exit 1`, and the previous backend version continues serving traffic because the long-running ECS service has `lifecycle.ignore_changes = [task_definition]` and was therefore never asked to roll over.

```bash
# Manually run the production migration task. The exact cluster name,
# task-definition family, subnets, and security groups are produced by
# `terraform output` against the prod workspace; they follow the
# pattern `sales-connections-<env>-cluster` and `sales-connections-<env>-migration`.
#
# Resolve the values once:
#   cluster_name=$(terraform output -raw ecs_cluster_name)
#   migration_family=$(terraform output -raw ecs_migration_task_definition_family)
#   ecs_sg=$(terraform output -raw ecs_security_group_id)
#   subnets=$(terraform output -json private_subnet_ids | jq -r 'join(",")')

aws ecs run-task \
  --cluster sales-connections-prod-cluster \
  --task-definition sales-connections-prod-migration \
  --launch-type FARGATE \
  --network-configuration "awsvpcConfiguration={subnets=[subnet-...],securityGroups=[sg-...],assignPublicIp=DISABLED}" \
  --propagate-tags TASK_DEFINITION \
  --started-by "operator-manual-migration"
```

The migration task uses the same backend image as the long-running service (the `terraform apply` step that ran ahead of this command registered the task-definition revision with the new image SHA). The task runs `alembic upgrade head` and exits 0 on success or non-zero on failure; the exit code is visible via:
...
Migration logs stream to CloudWatch with `awslogs-stream-prefix=migration` in the same log group as the backend service.
...
# Run the alembic downgrade as a one-shot task. The container `name`
# inside the override block MUST match the migration task's container
# name ("migration"); the command override replaces the task
# definition's default `alembic upgrade head` with the downgrade.
aws ecs run-task \
  --cluster sales-connections-prod-cluster \
  --task-definition sales-connections-prod-migration \
  --launch-type FARGATE \
