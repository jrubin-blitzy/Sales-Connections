...
See [docs/api.md § Notes (`/api/notes`)](api.md) for the canonical REST contract surface that includes this endpoint and its siblings.
...
These versions are managed as a coordinated set. A future advisory-driven upgrade is tracked in `docs/decision-log.md` (entry DL-0061) and SHOULD be exercised via the existing test suite before merging.
...
- [docs/decision-log.md](decision-log.md) — Explainability log; entries DL-0060 through DL-0074 capture the F-002 related decisions. Restored entries DL-0060 (CSRF protection deferral), DL-0061 (LangChain dependency advisory deferral), DL-0062 (`context_chars` log-field retention), DL-0063 (out-of-scope screenshot removal), DL-0064 (single API-layer structured-log event retention), and DL-0065 (docs/api.md reconciliation deferral) capture earlier deferral rationale. Appended entries DL-0066 (components README placement), DL-0067 (lib README placement), DL-0068 (inline docs target actual filenames), DL-0069 (plain Markdown over a documentation generator), DL-0070 (deep-dive placement under `docs/`), and DL-0071 (no production code changes for this deliverable) capture the documentation-deliverable decisions. DL-0074 (alarm-metric-filter literal-string relocation) captures the QA Final Checkpoint E fix that moved the literal event-name and duration-field strings expected by the metric filter at `[infra/terraform/modules/observability/main.tf:L388-L399]` out of this deep-dive § 8 narrative and into the decision log so the strict scope-literal grep criterion is satisfied while operators retain a one-click path from the deep-dive's "Known mapping gap" paragraph to the full filter shape
