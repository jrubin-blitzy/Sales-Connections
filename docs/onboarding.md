...
Today, the AI orchestrator subsumes every non-timeout, non-misconfiguration failure under `AIServiceUnavailableError(code="ai_unavailable", status_code=502)`. A future enhancement could introduce a dedicated `AIPromptTooLargeError` to give callers a more specific signal when the prompt context exceeds `AI_PROMPT_CONTEXT_MAX_CHARS` after sanitization or when the Claude API returns a 413-equivalent. The SPA could then surface a tailored "your input is too long; try shortening" affordance instead of the generic "AI is unavailable" banner. Implementation: add the new exception class in `backend/app/services/ai_orchestration.py`, map it through the existing `AppError` handler, and add a new `AiNoteErrorCode` value plus a `isPromptTooLarge` helper in `frontend/src/api/notes.ts`. Out of scope for F-002 documentation per the minimal-change clause.
