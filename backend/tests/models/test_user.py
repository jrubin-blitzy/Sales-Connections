...
class TestUserCreation:
    ...
    def test_default_role_is_contributor(self, db_session: DBSession, organization) -> None:
    ...
    def test_oauth_user_no_password_hash(self, db_session: DBSession, organization) -> None:
    ...
    def test_password_user_with_password_hash(self, db_session: DBSession, organization) -> None:
        """Password users have a non-null password_hash."""
        user = User(
            org_id=organization.id,
            email=f"pw-only-{uuid.uuid4().hex[:6]}@example.com",
            display_name="Password User",
            password_hash=("$2b$04$abcd1234567890abcd1234567890abcd1234567890abcd1234567"),
        )
        db_session.add(user)
        db_session.commit()
...
class TestUserConstraints:
    ...
    def test_composite_unique_org_email(self, db_session: DBSession, organization) -> None:
    ...
    def test_same_email_different_orgs_allowed(self, db_session: DBSession, organization) -> None:
        ...
        db_session.commit()
        # Both succeed.
        users = db_session.query(User).filter_by(email=common_email).all()
        assert len(users) == 2

    def test_email_required(self, db_session: DBSession, organization) -> None:
        """email is NOT NULL."""
        user = User(
            org_id=organization.id,
...
class TestUserRepr:
    ...
    def test_repr_excludes_email(self, db_session: DBSession, organization) -> None:
    ...
    def test_repr_excludes_password_hash(self, db_session: DBSession, organization) -> None:
    ...
    def test_repr_includes_class_name(self, db_session: DBSession, organization) -> None:
...
class TestUserRelationships:
    ...
    def test_organization_relationship(self, db_session: DBSession, organization) -> None:
